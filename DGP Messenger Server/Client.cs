...
{	
	public class Client
	{
		...
		public event LineRecivedEventHandler LineRecived;
		...
		{
			Logg("Sending Message: \'" + message.Replace("\r", "\n") + "\' To: " + this.Id);
			try
			{
		...
		private void MessageRecieved(IAsyncResult ar)
		{
			try
			{
				...
				// new data can be Recived later. We then loop through each message
				...
				while ((pos = messageBuffer.IndexOf("\n\r")) != -1)
				{
					...
					// We Recived a message, fire the event
					OnLineRecived(new LineRecivedEventArgs(this, message));
				}
			...
			catch (SocketException)
			{
				...
				OnLineRecived(new LineRecivedEventArgs(this, "LOST"));
			}
		}

		private void OnLineRecived(LineRecivedEventArgs e)
		{
	...
	//				Logg("Got contact Id! " + fileTransferId);
